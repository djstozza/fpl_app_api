...
RSpec.describe InterTeamTradeGroups::Decline do
  it 'declines the inter_team_trade_group' do
    ...
    expect(result.success).to eq("You have successfully declined #{out_fpl_team.user.username}'s trade proposal.")
    expect(inter_team_trade_group).to be_declined
  end
  ...
  it '#inter_team_trade_group_submitted' do
    ...
    expect(outcome.errors.full_messages).to contain_exactly("You can only decline submitted trade proposals.")
  end
end
