...
RSpec.describe WaiverPicks::Delete do
  it 'deletes the waiver_pick and reorders the remaining ones' do
  ...
  it '#round_is_current' do
    ...
    expect(fpl_team_list).to receive(:waiver_picks).and_return([waiver_pick]).at_least(1)
  ...
  it '#fpl_team_list_waiver_pick' do
    ...
    expect(fpl_team_list).to receive(:waiver_picks).and_return([waiver_pick_2]).at_least(1)
  ...
  it '#pending_waiver_pick' do
    ...
    expect(outcome.errors.full_messages).to contain_exactly("You can only delete pending waiver picks.")
  end
end
