class FplTeams::Score < ApplicationInteraction
  ...
  object :fpl_team_list, class: FplTeamList, default: -> { fpl_team.fpl_team_lists.find_by(round: round) }
  ...
  def counted(instance)
    instance.where(
      "fixtures.finished IS FALSE OR (fixtures.finished IS TRUE AND (fixture_history ->> 'minutes')::INT > ?)", 0
    )
  end
